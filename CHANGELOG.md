...
- **A subscription now ends only when the consuming execution terminates** — `end` or a `result` — **or is cancelled or fails.** "Leaving for an unrelated part of the graph" is no longer a way out, because there is no longer any such thing: the activities a handler transitions through are the handler. Stopping early is `end` or a `result`, which is what the one existing escape already did
- **A promise branch `target` may name any activity**, and a non-workflow target runs the same workflow from that activity. The spec never said otherwise — `PromiseBranch.target` asks only "which activity to start", `PromiseActivityConfig` already documents targets that name a promise or an ancestor, and no `UTOS-*` rule constrains the kind — but the reference daemon threw unless the target was a `workflow.call`. That made fanning out over an HTTP request require a second document wrapping it, and, worse, made the daemon reject bundles the shared validator accepts. That every branch is a separate execution is an implementation's fan-out strategy, not a shape an author must arrange
