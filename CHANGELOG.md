...
- This **supersedes the 0.0.13 relaxation** that let a branch `target` name any activity, which is why that entry is gone rather than reversed here. It fixed a real validator/daemon divergence — the daemon threw unless the target was a `workflow.call` — and this fixes the same divergence the other way, by putting the rule in the spec where it was always missing. The capability it added is not lost: fanning out over an activity in the current document is now `workflow: self`
