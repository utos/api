...
4. In every rule — `onSuccess`, `onFailure`, `onEmitted` — a `return` key is renamed to
   ...
   `return:` with nothing after it, `return: ~` or `return: null` — or, for a rule with no
   condition, the bare list item `- return`, which YAML reads as the string `return` and this
   mapping reads as the whole rule. The wire keeps the name `result` because `return` is a reserved
